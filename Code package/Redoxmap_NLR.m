function varargout = Redoxmap_NLR(varargin)
% REDOXMAP_NLR MATLAB code for Redoxmap_NLR.fig
%      REDOXMAP_NLR, by itself, creates a new REDOXMAP_NLR or raises the existing
%      singleton*.
%
%      H = REDOXMAP_NLR returns the handle to a new REDOXMAP_NLR or the handle to
%      the existing singleton*.
%
%      REDOXMAP_NLR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in REDOXMAP_NLR.M with the given input arguments.
%
%      REDOXMAP_NLR('Property','Value',...) creates a new REDOXMAP_NLR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Redoxmap_NLR_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Redoxmap_NLR_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Redoxmap_NLR

% Last Modified by GUIDE v2.5 28-Sep-2021 08:17:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Redoxmap_NLR_OpeningFcn, ...
                   'gui_OutputFcn',  @Redoxmap_NLR_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Redoxmap_NLR is made visible.
function Redoxmap_NLR_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Redoxmap_NLR (see VARARGIN)

% Choose default command line output for Redoxmap_NLR
...
% UIWAIT makes Redoxmap_NLR wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = Redoxmap_NLR_OutputFcn(hObject, eventdata, handles) 
...
handles.redox.xanespath=xanespath;
handles.redox.xanesfilename=xanesfilename;
...
% global xrfff2
% global xrffp
xanesfilename=handles.redox.xanesfilename;
xanespath=handles.redox.xanespath;
cd(xanespath);
...
S1=Uf1(i);
S2=Uf2(i);
d=[xaneserff1,xaneserf2f1;xaneserff2,xaneserf2f2];
d1=[S1,xaneserf2f1;S2,xaneserf2f2];
d2=[xaneserff1 S1; xaneserff2 S2];
...
Uratio3=(U4*oxs+U5*oxss)./(U4+U5);





scatter(XRFX, XRFY, 40 , Uratio3 ,'s', 'filled');
axis off;
set(gca,'tickdir','out');
colormap jet;
colorbar;
